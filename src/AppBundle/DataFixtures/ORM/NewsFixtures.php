...
/**
 * Defines the sample blog news to load in the database before running the unit
...
class PostFixtures extends AbstractFixture implements DependentFixtureInterface, ContainerAwareInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        foreach ($phrases as $i => $title) {
            ...
            $news->setTitle($title);
            $news->setExcerpt($this->getRandomPostSummary());
            $news->setSlug($this->container->get('slugger')->slugify($news->getTitle()));
            $news->setContent($this->getPostContent());
            ...
            $news->setAuthor($this->getReference('admin1'));
            $news->setCreated(new \DateTime('now - '.$i.'days'));

            $this->addRandomTags($news);

            foreach (range(1, 5) as $j) {
                $comment = new Comment();

                $comment->setAuthor($this->getReference('user'));
                $comment->setPublished(new \DateTime('now + '.($i + $j).'seconds'));
    ...
    private function addRandomTags(News $news)
    {
